...
export default async (debug = false, config, settings, is_replit = (process.env.REPL_ID && process.env.REPL_SLUG && process.env.REPL_OWNER)) => {
  ...
  const client = new Client({
    auth: "Bot " + await config.get("token"),
  ...
  client.connect().catch(spinner ? (e) => spinner.error({ text: `The bot did not work and the error is: ${e.message}` }) : console.error);
  ...
  client.on("ready", async () => {
    ...
    setInterval(async () => {
      client.editStatus(`online`, [{ name: await settings.get("status_bot") || "Bot By Shuruhatik", type: await settings.get("status_type") || 0 }]);
    }, 30000);
            ...
            components: [
              {
                ...
                components: [
                  ...
                  },
                  {
                    ...
                    url: "https://discord.gg/bwt-fdhkrwny-729526735749513267",
                    style: 5,
                  }
                ]
              }
